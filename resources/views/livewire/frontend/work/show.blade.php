                ...
                <div class="col-span-full xl:col-span-3">
                        ...
                        <div class="bg-brand-blue px-4 py-2 rounded-full absolute top-[20px] right-[20px]">
                            <div class="flex items-center gap-1">
                                ...
                                <p class="text-white text-sm">
                                    <span class="font-semibold">{{ $work->profile->rating() }}</span>
                                    ({{ $work->profile->ratings->count() }})
                                </p>
                            </div>
                        </div>
                    ...
                    @if($work->attachment_file_names != null)
                        <div class="bg-white p-6 rounded-xl mt-5">
                            <h6 class="text-lg font-semibold mb-3">{{ __('general.attachments') }}</h6>
                            <div class="flex items-center gap-3 flex-wrap">
                                @foreach($work->attachment_file_names as $attachment => $name)
                                    <a wire:click="$dispatch('download-attachment', $attachment)"
                                       class="px-3 py-1 border border-secondary-1 text-sm rounded-full cursor-pointer flex items-center justify-center space-x-2">
                    ...
                    @if($work->skills->count() > 0)
                        <div class="bg-white p-6 rounded-xl mt-5">
                            <h6 class="text-lg font-semibold mb-3">{{ __('general.skills') }}</h6>
                            <div class="flex items-center gap-3 flex-wrap">
                    ...
                    @if($work->workTags->count() > 0)
                        <div class="bg-white p-6 rounded-xl mt-5">
                            <h6 class="text-lg font-semibold mb-3">{{ __('general.tags') }}</h6>
                            <div class="flex items-center gap-3 flex-wrap">
                                @foreach($work->workTags as $tag)
                                    <a href="{{ route('works.tag', $tag) }}"
                                       class="px-3 py-1 border border-secondary-1 text-sm rounded-full cursor-pointer">{{ $tag->name }}</a>
                                @endforeach
                            </div>
                        </div>
                    @endif
